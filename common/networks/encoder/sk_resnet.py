...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes: int, planes: int, stride: int = 1):
        ...
        self.conv1 = SKConv(in_planes, planes, kernel_size=[3, 5], stride=stride)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes: int, planes: int, stride: int = 1):
        ...
        self.bn1 = nn.BatchNorm2d(planes)
        self.act1 = nn.ReLU(inplace=True)
        self.conv2 = SKConv(planes, planes, kernel_size=[3, 5], stride=stride)
...
class SKResNetEncoder(nn.Module):
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        for idx, stride in enumerate(strides):
            layers.append((f"block{idx+1}", block(self.in_planes, planes, stride)))
